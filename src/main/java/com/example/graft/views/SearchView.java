...
public class SearchView extends VerticalLayout {
    ...
    private List<SearchUserDTO> allData;
    ...
    public SearchView(UserRepository repo, String currentUsername) {
        this.repo = repo;
        ...
        searchLayout.getStyle().set("margin-bottom", "1.5em");
        ...
        var searchButton = new Button("Search", new Icon(VaadinIcon.SEARCH));
        ...
        section.add(searchLayout);

        // Results count container
        var resultsHeaderLayout = new HorizontalLayout();
        resultsHeaderLayout.setWidthFull();
        resultsHeaderLayout.setJustifyContentMode(JustifyContentMode.CENTER);
        resultsHeaderLayout.setAlignItems(Alignment.CENTER);
        
        resultsCountSpan = new Span();
        resultsCountSpan.getStyle()
            .set("font-family", "'Poppins', sans-serif")
            .set("font-weight", "600")
            .set("color", "#666666");
            
        resultsHeaderLayout.add(resultsCountSpan);
        section.add(resultsHeaderLayout);
    ...
    private void performSearch(String term) {
        ...
        try {
            ...
            String searchTermNoSpace = searchTerm.replaceAll("\\s+", "");
            allData = repo.searchUsersComprehensive(searchTerm, searchTermNoSpace);
            ...
            if (allData.isEmpty()) {
                ...
                var noResults = new Paragraph("No users found.");
                noResults.getStyle()
                ...
                resultsCountSpan.setText("Found " + allData.size() + " users.");
                currentPage = 1;
                renderPage();
            }
        } catch (Exception e) {
            System.err.println("DEBUG: Error searching users: " + e.getMessage());
            e.printStackTrace();
            var errorMsg = new Paragraph("Error searching users: " + e.getMessage());
            errorMsg.getStyle()
                .set("font-family", "'Nunito', sans-serif")
                .set("color", "#d32f2f");
    ...
    private void renderPage() {
        ...
        for (int i = startIndex; i < endIndex; i++) {
            SearchUserDTO user = allData.get(i);
        ...
        var viewProfileBtn = new Button("View Profile", event -> {
            try {
                ...
                if (userOptional.isPresent()) {
                    ...
                    showUserProfileDialog(userOptional.get(), effectiveUserId, followerCount);
                } else {
                    System.out.println("  User not found for username/userId");
                }
            } catch (Exception e) {
                System.err.println("DEBUG: Error in SearchView View Profile button: " + e.getMessage());
                e.printStackTrace();
            }
        });
